...
A set of integration tests to apply to the FpyDemo app. This is intended to be a reference of integration testing.
"""
...
def test_is_streaming(fprime_test_api):
    ...
    "FpyDemo.sendBuffComp.SendState" is verified to be SEND_IDLE.
    """
    ...
    fprime_test_api.assert_telemetry(
        "FpyDemo.sendBuffComp.SendState", value="SEND_IDLE", timeout=3
    )
...
def test_bd_cycles_ascending(fprime_test_api):
    ...
    results = fprime_test_api.await_telemetry_count(
        count_pred, "FpyDemo.blockDrv.BD_Cycles", timeout=length
    )
    last = None
    reordered = False
    ascending = True
...
def test_signal_generation(fprime_test_api):
    ...
    fprime_test_api.send_and_assert_command("FpyDemo.SG4.Settings", [1, 5, 0, "SQUARE"])
    ...
    fprime_test_api.send_and_assert_command("FpyDemo.SG4.Toggle")
    fprime_test_api.assert_telemetry("FpyDemo.SG4.History", history, timeout=6)
    fprime_test_api.assert_telemetry("FpyDemo.SG4.PairHistory", pair_history, timeout=3)
    fprime_test_api.assert_telemetry("FpyDemo.SG4.Info", info, timeout=3)
    fprime_test_api.send_and_assert_command("FpyDemo.SG4.Toggle")
...
def test_seqgen(fprime_test_api):
    ...
    fprime_test_api.send_and_assert_command(
        "ComFpy.cmdSeq.CS_RUN", args=["/tmp/ref_test_int.bin", "BLOCK"], max_delay=5
    )
